...
ggplot(data = full_data, aes(x = siteID, # name the variable that goes on the x-axis
  ...
  geom_jitter(width = 0.05, alpha = 0.2) +# Graph these variables using boxplots
  theme_minimal() +
  ggtitle("Total count per site") +
...
ggplot(data = full_data, aes(x = siteID, # name the variable that goes on the x-axis
  ...
  geom_boxplot(alpha=.5) +  # Graph these variables using boxplots
  theme_minimal()  +
  ggtitle("Alpha Diversity within Sites") +
  labs(x = "Site ID", # Specify axis labels
       y = "Alpha diversity",
       fill = "siteID")
```
