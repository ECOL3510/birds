...
#below is the code used to download the bird count data
```{r, eval = FALSE}
list2env((loadByProduct(dpID = 'DP1.10003.001',
                           site = c('SERC', 'UKFS', 'NOGP', 'STER'),
                           startdate = '2018-01',  # YYYY-MM
                           enddate = '2019-12')),  # YYYY-MM
         .GlobalEnv)
```
```{r}
data<-read_csv("./Bird_data/brd_countdata.csv")
```
...
for (i in 1:length(files)){
  ...
  write_csv(files[[i]], file = paste0("./Bird_data/", names(files[i]), ".csv", sep=""))
}
```
...
#below is the code used to download the NEON temperature data for each site
```{r, eval = FALSE}
...
for (i in 1:length(files)){
  # Change the SCBI_mammals text below to indicate the name of the folder where you want to store your data
  write_csv(files[[i]], file = paste0("./temperature_raw/", names(files[i]), ".csv", sep=""))
}
```